...
class recommender:
    ...
    def __init__(self):
        #这里选择路径
        # self.root_dir = "F:\programming\python\SoftwarePractice\Book-Crossing-Data-Cleansing-for-Recommendations-new"
        # self.dfs_path = "F:\programming\python\SoftwarePractice\Book-Crossing-Data-Cleansing-for-Recommendations-new\Data\datasets"
        # self.model_path = "F:\programming\python\SoftwarePractice\Book-Crossing-Data-Cleansing-for-Recommendations-new\Data\datasets\model.pickle"
    ...
    def create_new_user(self):
        ...
        self.user_Id = int(max(self.users_df.user_id))+1 
    ...
    def recommend(self, nof_rec=4, verbose=True):
        '''
        ...
        if verbose:
            ...
            for i, item in enumerate (items.iterrows()):
                ...
                print('{0}) "{1}({2})" from {3}. ({4})'.format(i+1, item[1][1], int(item[1][0]), item[1][2], int(top_n[i][1])))
        return items
    ...
    def rand_recommend(self, nof_rec=4, verbose=True):   # return top-n full-rating books randomly
        # get the books with the top predicted rating and construct a pd.DataFrame of them and the ratings
        ...
        if verbose:
            ...
            for i, item in enumerate(items.iterrows()):
                # print('Book "',item[1], '" from "', item[2], '", (%f)'%item[5])
                print('{0}) "{1}({2})" from {3}. '.format(i + 1, item[1][1], int(item[1][0]), item[1][2],))
        return items
